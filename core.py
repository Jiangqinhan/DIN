...
class DNN(Layer):
    """The Multi Layer Percetron
      ...
      Input shape
        ...
        在DIN的attention中则是(batch_size,input_len,embedding_dim)
    ...
    def build(self, input_shape):
        start_size = input_shape[-1]
        hidden_units = [int(start_size)] + list(self.hidden_units)
        ...
        self.bias = [self.add_weight(name='bias' + str(i), shape=(self.hidden_units[i],),
                                     initializer=Zeros(), trainable=True) for i in range(len(self.hidden_units))]
        if self.use_bn:
            self.bn_layer = [tf.keras.layers.BatchNormalization() for _ in range(len(self.hidden_units))]
...
class LocalActivationUnit(Layer):
    ...
    user interests varies adaptively given different candidate items.
      ...
      Input shape
        - A list of two 3D tensor with shape: query  ``(batch_size, 1, embedding_size)`` and  key ``(batch_size, T, embedding_size)``
    ...
    def build(self, input_shape):
        ...
        '''
        由于DNN允许0层fc的存在 所以这里设置如果hidden_units 长度为0时 也至少有一个全连接层
        '''
        size = 4 * input_shape[0][-1] if len(self.hidden_units) == 0 else self.hidden_units[-1]
        self.kernel = self.add_weight(name='kernel', shape=(size, 1), initializer=glorot_normal(seed=self.seed))
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        query = tf.tile(query, [1, keys_len, 1])
        att_input = tf.concat([query, keys, query - keys, query * keys], axis=-1)
